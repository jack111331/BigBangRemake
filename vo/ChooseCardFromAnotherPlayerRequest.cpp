...
void PlayerCard::Request::to_json(nlohmann::json &j, const ChooseCardFromAnotherPlayerRequest &p) {
    j = nlohmann::json{{"chosenPosition", p.chosenPosition},
                       {"amount",         p.amount},
                       {"cardList",       p.cardList}};
}

void PlayerCard::Request::from_json(const nlohmann::json &j, ChooseCardFromAnotherPlayerRequest &p) {
